...
class FineTuneCNN:
    ...
    def crop_generator(self,xy_gen, n_crops=5):
        """
        ...
        while True:
            x, y = next(xy_gen)
            batch_crops = np.zeros((n_crops, self.dimensions,  self.dimensions, 3))
            batch_ys = np.tile(y, (n_crops, 1))  # repeat 'y' 5 times
    ...
    def train(self, train_path=None, num_training_samples=None, validation_path=None, num_validation_samples=None, 
              batch_size=0, num_epochs=0, early_stopping=False, aug=False,crop=False, checkpoint=None, model_path=None):
        """Fits the model
        ...
        if validation_path:
            ...
            if self.crop:
                self.val_gen = crop_generator(self.val_gen)
        else:
            self.val_gen = None
    ...
    def test(self, test_path=None, num_test_samples=None, batch_size=32, crop=False):
        ...
        self.test_gen = test_datagen.flow_from_directory(self.test_path, 
                                                         ...
                                                         shuffle=False)
        if self.crop:
                self.test_gen = crop_generator(self.test_gen)
